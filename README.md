...
## Замечание - зачем разделение на `langchain-agent` и `langchain-agent-api`
...
Готовые к импорту потоки лежат в папке `examples/`. Все примеры под OpenAI-совместимый эндпоинт, для переключения достаточно изменить поле **Base URL** в `langchain-config`.
